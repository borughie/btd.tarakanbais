<div class="font-['Plus_Jakarta_Sans',sans-serif]">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
        <div class="bg-white rounded-2xl shadow-sm border border-[#B3202E]/10 border-t-4 border-t-[#B3202E] p-5">
            <p class="text-xs font-semibold uppercase tracking-wider text-[#6B5C5C]">Hari Ini</p>
            <p class="font-['Fraunces',serif] text-3xl font-bold text-[#B3202E] mt-1.5">{{ $todayCount }}</p>
        </div>
        <div class="bg-white rounded-2xl shadow-sm border border-[#B3202E]/10 border-t-4 border-t-[#241B1B] p-5">
            <p class="text-xs font-semibold uppercase tracking-wider text-[#6B5C5C]">Total Tamu</p>
            <p class="font-['Fraunces',serif] text-3xl font-bold text-[#241B1B] mt-1.5">{{ $totalCount }}</p>
        </div>
        <div class="bg-white rounded-2xl shadow-sm border border-[#B3202E]/10 border-t-4 border-t-[#C9A227] p-5">
            <p class="text-xs font-semibold uppercase tracking-wider text-[#6B5C5C]">Notifikasi Terkirim</p>
            <p class="font-['Fraunces',serif] text-3xl font-bold text-[#C9A227] mt-1.5">{{ $notifiedCount }}</p>
        </div>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border border-[#B3202E]/10 mb-6 overflow-hidden">
        <div class="p-4 border-b border-[#B3202E]/10 bg-[#FBEAEA]/30">
            <div class="flex flex-col sm:flex-row gap-3">
                <div class="flex-1">
                    <input type="text" wire:model.live="search" placeholder="Cari instansi atau tujuan..."
                        class="w-full px-4 py-2.5 border border-[#B3202E]/15 rounded-xl focus:ring-2 focus:ring-[#B3202E]/40 focus:border-[#B3202E] transition duration-200 outline-none text-[#241B1B] placeholder:text-gray-400 bg-white" />
                </div>
                <div class="flex gap-3">
                    <input type="date" wire:model.live="dateFrom"
                        class="px-4 py-2.5 border border-[#B3202E]/15 rounded-xl focus:ring-2 focus:ring-[#B3202E]/40 focus:border-[#B3202E] transition duration-200 outline-none text-[#241B1B] bg-white" />
                            ...
                            <td colspan="8" class="px-6 py-16 text-center">
                                <div class="text-[#6B5C5C]">
                                    <div
                                        ...
                                        <svg class="w-7 h-7 text-[#B3202E]" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                                d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" />
                                        </svg>
                                    </div>
                                    <p class="font-['Fraunces',serif] text-lg font-bold text-[#241B1B]">Belum Ada Data
                                        Tamu</p>
        ...
        @if ($guests->hasPages())
            <div class="px-6 py-4 border-t border-[#B3202E]/10">
                {{ $guests->links() }}
            </div>
        @endif
    </div>

    {{-- QR Modal: seal/document motif consistent with guest page --}}
    @if ($showQrModal)
                ...
                <div class="relative bg-white rounded-[28px] shadow-2xl max-w-sm w-full overflow-hidden" wire:click.stop>
                    ...
                    <div class="px-8 py-7 text-center">
                        ...
                        <div class="bg-[#FBEAEA]/40 rounded-2xl p-4 inline-block mb-4 border border-[#B3202E]/10">
                            <img src="{{ $this->qrDataUri }}" alt="QR Code Buku Tamu" class="w-56 h-56" />
                        </div>
